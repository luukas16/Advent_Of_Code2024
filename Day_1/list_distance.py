...
As each location is checked, they will mark it on their list with a star. They figure the Chief Historian must be in one of the first fifty places they'll look, so in 
...
Upon pouring into the office, everyone confirms that the Chief Historian is indeed nowhere to be found. Instead, the Elves discover an assortment of notes and lists 
of historically significant locations! This seems to be the planning the Chief Historian was doing before he left. Perhaps these notes can be used to determine which 
locations to search?
...
def list_distance(arr1, arr2):
    # First we sort the arrays so that they are in descending order
    # Makes it easy to pair up elements for finding distnace between elements
    arr1.sort()
    arr2.sort()
    
    # A placeholder value to store the end resulting distance
    distance = 0
    
    # Now that we have sorted arrays we can begin matching up elements starting at index 0
    # Ill also throw in an error check to see if the sizes of the arrays are the same
    if len(arr1) == len(arr2):
        for i in range(len(arr1)):
            # We can now begin the distance logic.
            # Its possible we end up subtracting a larger number from a smaller one 
            # Which results in a negative value. For this I see two possible solutions:
            # 1: Use the abs() function to quickly get the correct positive distance
            # 2: Or do some if else logic to ensure a positive result
